...
//        //System.out.println(JSONUtil.parseObj(body).get("errno").toString());
    ...
    @Test
    void d() {
        String str = HttpUtil.get("https://cache.thepaper.cn/contentapi/node/getWwwAllNodes");
        JsonElement jsonElement = JsonParser.parseString(str).getAsJsonObject().get("data");
        JsonElement channelList = jsonElement.getAsJsonObject().get("channelList");
        JsonArray asJsonArray = channelList.getAsJsonArray();
        List<String> list = Arrays.asList("时事",
                "国际",
                "财经",
                "科技",
                "智库",
                "思想",
                "生活");
        List< HotApi> hotapi=new ArrayList<>();
        for (int i = 0; i < asJsonArray.size(); i++) {
            ...
            if (list.contains(asJsonObject.get("name").getAsString())) {
                if (!asJsonObject.has("childNodeList")) {
                    continue;
                }
                JsonArray childNodeList = asJsonObject.get("childNodeList").getAsJsonArray();
                for (JsonElement element : childNodeList) {
                    String nodeId = element.getAsJsonObject().get("nodeId").getAsString();
                    String name = element.getAsJsonObject().get("name").getAsString();
                    String bigDataCode = element.getAsJsonObject().get("bigDataCode").getAsString();
                    System.out.printf("%s : %s : %s%n", nodeId, name, bigDataCode);
                    HotApi hotApi = new HotApi();
                    hotApi.setPlatform("thepaper_"+bigDataCode);
                    hotApi.setApiName("澎湃"+name);
                    hotApi.setApiURL("https://www.thepaper.cn/_next/data/576df16391327e0cc3c3078a779cdf93fb443968/list_%s.json");
                    hotApi.setApiParam(nodeId);
                    hotApi.setApiDescribe("澎湃"+name+"热点");
                    hotapi.add(hotApi);
                }
            }
        }
        hotApiService.saveBatch(hotapi);
    }
